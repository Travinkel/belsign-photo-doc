...
public class LoginViewModel extends BaseViewModel<LoginViewModel> {
    ...
    /**
     * Creates a new LoginViewModel with the default AuthenticationService and a new RoleBasedNavigationService.
     */
    public LoginViewModel() {
        // Get the SessionContext from ServiceLocator
        ...
        // Get the AuthenticationService from ServiceLocator
        authenticationService = ServiceLocator.getService(AuthenticationService.class);
    }

    /**
     * Creates a new LoginViewModel with the specified AuthenticationService and RoleBasedNavigationService.
     * This constructor is primarily used for testing.
     *
     * @param authenticationService the authentication service
     * @param navigationService the role-based navigation service
     */
    public LoginViewModel(AuthenticationService authenticationService, RoleBasedNavigationService navigationService) {
    ...
    public void login() {
        ...
        try {
            // Attempt to log in using AuthenticationService
            logger.debug("Calling authenticationService.authenticate for user: {}", username.get());
            Optional<UserBusiness> userOpt = authenticationService.authenticate(username.get(), password.get());

            if (userOpt.isPresent()) {
                // Login successful, set the current user in SessionContext
    ...
    public void loginWithPin(String pinCode) {
        ...
        try {
            ...
            Optional<UserBusiness> userOpt = Optional.empty();

            // Check if we have an ExtendedAuthenticationService that supports PIN authentication
            if (authenticationService instanceof com.belman.domain.security.ExtendedAuthenticationService) {
                // Cast to ExtendedAuthenticationService to access PIN authentication methods
                com.belman.domain.security.ExtendedAuthenticationService extendedAuthService = 
                    (com.belman.domain.security.ExtendedAuthenticationService) authenticationService;

                // Try to authenticate with PIN if the method is available
                try {
                    // This is a workaround since authenticateWithPin is not in the interface
                    // but might be available in the implementation
                    java.lang.reflect.Method method = extendedAuthService.getClass()
                        .getMethod("authenticateWithPin", String.class);
                    userOpt = (Optional<UserBusiness>) method.invoke(extendedAuthService, pinCode);
                    logger.debug("Called authenticateWithPin via reflection");
                } catch (NoSuchMethodException e) {
                    logger.warn("authenticateWithPin method not found in ExtendedAuthenticationService");
                    errorMessage.set("PIN authentication is not supported in this version.");
                    return;
                } catch (Exception e) {
                    logger.error("Error calling authenticateWithPin", e);
                    throw e;
                }
            } else {
                logger.warn("ExtendedAuthenticationService not available for PIN authentication");
                errorMessage.set("PIN authentication is not supported in this version.");
                return;
            }

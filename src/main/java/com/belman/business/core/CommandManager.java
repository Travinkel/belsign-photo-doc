...
import com.belman.business.richbe.events.CommandExecutedEvent;
import com.belman.business.richbe.events.CommandRedoneEvent;
import com.belman.business.richbe.events.CommandUndoneEvent;
        ...
        return command.execute()
                .thenApply(result -> {
                    ...
                    if (command.canUndo()) {
                        ...
                        eventPublisher.publish(new CommandExecutedEvent(command));
                    }
                    return result;
                })
                .exceptionally(ex -> {
    ...
    public CompletableFuture<Void> undo() {
        ...
        return command.undo()
                .thenAccept(v -> {
                    ...
                    eventPublisher.publish(new CommandUndoneEvent(command));
                })
                .exceptionally(ex -> {
    ...
    public CompletableFuture<?> redo() {
        ...
        return command.execute()
                .thenApply(result -> {
                    ...
                    eventPublisher.publish(new CommandRedoneEvent(command));
                    return result;
                })
                .exceptionally(ex -> {

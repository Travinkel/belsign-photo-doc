...
public class RoleBasedAccessManager extends DomainService {
    ...
    public boolean hasAccess() {
        Optional<UserBusiness> currentUser = authenticationService.getCurrentUser();
     ...
     */
    public void checkAccess(UserBusiness user) throws AccessDeniedException {
        if (!hasAccess(user)) {
            throw new AccessDeniedException("Access denied. User does not have the required role.");
        }
    }
     ...
     */
    public boolean hasAccess(UserBusiness user) {

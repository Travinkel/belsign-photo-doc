...
/**
 * Utility class for initializing the database with data useful for the RAT (Requirements Acceptance Testing) phase.
...
public class RatPhaseDataInitializer {
    ...
    public static void populateRatPhaseData() {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            try {
                ...
                connection.commit();
                LOGGER.info("RAT phase test data populated successfully");
            } catch (SQLException e) {
                // Rollback the transaction if an error occurs
                connection.rollback();
                LOGGER.log(Level.SEVERE, "Error populating RAT phase test data", e);
                throw e;
            } finally {
            ...
            throw new RuntimeException("Failed to populate RAT phase test data", e);
        }
    }
    ...
    private static void addTestPhotoTemplates(Connection connection) throws SQLException {
        ...
        addPhotoTemplate(connection, requiredTemplateId, "RAT_REQUIRED_VIEW", "Required view for RAT testing", true);

        // Add an optional photo template
        String optionalTemplateId = UUID.randomUUID().toString();
        addPhotoTemplate(connection, optionalTemplateId, "RAT_OPTIONAL_VIEW", "Optional view for RAT testing", false);
    ...
    private static void addTestAnnotations(Connection connection) throws SQLException {
        ...
        addAnnotation(connection, annotationId, photoId, 100.0, 100.0, 50.0, 50.0, "RAT phase test annotation", null);
    ...
    private static void addTestReports(Connection connection) throws SQLException {
        ...
        addReport(connection, pendingReportId, orderId, "RAT Pending Report", "Report for RAT testing (pending)", "PENDING", null);

        // Add an approved report
        String approvedReportId = UUID.randomUUID().toString();
        addReport(connection, approvedReportId, orderId, "RAT Approved Report", "Report for RAT testing (approved)", "APPROVED", null);

        // Add a rejected report
        String rejectedReportId = UUID.randomUUID().toString();
        addReport(connection, rejectedReportId, orderId, "RAT Rejected Report", "Report for RAT testing (rejected)", "REJECTED", null);

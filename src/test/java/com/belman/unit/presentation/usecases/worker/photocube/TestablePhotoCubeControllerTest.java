...
/**
 * Testable unit tests for the PhotoCubeViewController class.
 * These tests focus on the controller's logic without relying on JavaFX components.
 * 
 * Instead of trying to mock JavaFX components, which is challenging in a standard
 * JUnit test environment, this test uses a custom TestablePhotoCubeViewModel
 * and reflection to directly invoke the methods that contain the logic we want to test.
 */
public class TestablePhotoCubeControllerTest {
    ...
    /**
     * Helper method that simulates the handleShowRemainingToggle method from the controller.
     * 
     * @param controller the controller
     * @param viewModel the view model
     */
    private void simulateShowRemainingToggle(PhotoCubeViewController controller, TestablePhotoCubeViewModel viewModel) throws Exception {
        // Get the showRemainingToggle field
        Field showRemainingToggleField = PhotoCubeViewController.class.getDeclaredField("showRemainingToggle");
        showRemainingToggleField.setAccessible(true);
        javafx.scene.control.CheckBox showRemainingToggle = (javafx.scene.control.CheckBox) showRemainingToggleField.get(controller);
